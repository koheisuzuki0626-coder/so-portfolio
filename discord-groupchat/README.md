...
### エージェントモード（Claude Codeをツール付きで動かす・承認はDiscordで）
```
!agent このフォルダのファイル一覧を出して
```
Claude がコマンド実行・ファイル編集・Web検索などのツールを使う前に、Discord に
**[✅ 許可] [❌ 拒否]** ボタンが出ます。押した結果が Claude Code に返ります
（読み取り系ツールは自動許可）。ボタンは `!agent` を実行した本人のみ操作可、5分で自動拒否。

> ⚠️ 許可するとあなたのMac上でコマンド/編集が実行されます。信頼できる相手のいる
> サーバーでのみ使い、内容を確認してから [✅許可] を押してください。
