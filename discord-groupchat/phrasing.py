...
# 「こう/そう/ああ/どう + やって」は様子を指す副詞。依頼の「やって」と紛れる。
_KOU_YATTE_RE = re.compile("(こう|そう|ああ|どう|こんな風に|そんな風に)やって")
...
def _wants_action(text):
    ...
    if not t:
        return False
    # 「こうやって」「どうやって」の中の【やって】は副詞の一部で、依頼ではない。
    # 事故（2026-09-18）：銀行の画面のスクショに「こうやって出る」と添えただけで
    # 動画生成が始まった。依頼の形を数え上げる側の取りこぼし。
    t = _KOU_YATTE_RE.sub("", t)
    if not t.strip():
        return False
...
# 【文字】を書いてほしい依頼。絵や映像ではなくテキストが成果物のもの。
# 事故（2026-09-02）：NotebookLMの入力欄のスクショを添付して
# 「ここの文章を考えて欲しい」と送ったら、画像添付だけを根拠に生成へ流れ、
# クレジットを使う動画生成が始まった。添付は「これで何かして」の意思表示だが、
# 何をしてほしいかは本文で決まる。文章を求められているなら、その添付は
# 素材ではなく“読む対象”。
# 語を数え上げる書き方だが、ここは「成果物が何か」という名詞の話なので
# 言い回しの無限性は問題になりにくい（動詞側は _wants_action に任せる）。
_WANTS_TEXT_RE = re.compile(
    r"(文章|文言|プロンプト|コピー|キャッチ|台本|原稿|セリフ|字幕|"
    r"タイトル|見出し|説明文|キャプション|要約|翻訳|訳し|文面|例文)"
    r"[をはがにも]?\s*"
    r"(考え|書い|作っ|作成|練っ|直し|整え|まとめ|提案|出し|教え|し(て|た)"
    r"|化して)"
)
