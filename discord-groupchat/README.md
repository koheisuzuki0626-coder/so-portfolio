...
### 音声ファイルの書き起こし（Gemini・無料枠）
- チャンネルに音声ファイルを添付すると、Gemini が内容を**自動で書き起こして投稿**します。
- 対応形式: mp3 / wav / m4a / aac / ogg / opus / flac（Discordのボイスメッセージも対応）。上限 20MB。
- 複数話者は「話者A:」「話者B:」で区別し、末尾に【要約】が付きます。
- 音声だけ添付 → 書き起こしのみ投稿。テキストを添えて添付 → 書き起こし内容を踏まえてAIが回答。
- 書き起こしは会話履歴にも残るので、後から「さっきの音声の〜」と質問できます。
