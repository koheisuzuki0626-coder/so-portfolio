...
社名の「.」を塗り円で描き、録画ボタンの ● として読ませている。
サイトのヘッダーとフッターでは **CSS で描いている**(`.logo-dot`)ので、
文字のピリオドに戻すとロゴでなくなる。e2e で図形であることを固定してある。

| ファイル | 用途 |
|----------|------|
| `assets/logo.svg` | 明るい地のワードマーク。名刺・資料・印刷用 |
| `assets/logo-dark.svg` | 暗い地に載せるとき。点も明るい金に振ってある |
| `assets/favicon.svg` | ブラウザのタブ。角丸の黒地にマークを収めたもの |
| `assets/apple-touch-icon.png` | iOS のホーム画面用(180×180) |
| `assets/ogp.png` | SNS 共有時の画像(1200×630) |

`logo.svg` / `logo-dark.svg` は文字をアウトライン化してあるので、
Inter が入っていない環境や印刷所でも同じ形で出る。
中身の比率(点の大きさ・位置)は CSS と揃えてあり、
どちらかだけ変えると e2e が落ちる。作り直す場合は `.logo-dot` の
`width` / `margin-left` / `transform` と、フォントサイズ 100 換算の数値を合わせること。
