...
`about.html` の `#company` 内に **HTML コメントのまま置いてある**。
値が決まったらコメントを外すだけで正しい位置に出る（並び順は決定済み）。

- 代表者（氏名）
- 開業（年月）
- 所在地（**市区町村まで**の方針。番地は載せない）
- 電話番号
- 営業時間・定休日
...
`index.html` の JSON-LD（`Organization`）には住所・電話・設立年を**入れていない**。
確定していない情報を検索エンジンに渡さないため。会社概要が埋まったら、
JSON-LD にも `address` / `telephone` / `foundingDate` を足し、
e2e の「構造化データに未確定の情報を入れていない」を実態に合わせて書き換えること。
